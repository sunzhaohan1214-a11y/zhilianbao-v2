...
describe("M3-007 system governance pure contracts", () => {
  ...
  it("requires an explicit test-only fake-provider gate", () => { expect(fakeSystemProvidersEnabled({ APP_ENV: "test" })).toBe(false); expect(fakeSystemProvidersEnabled({ APP_ENV: "test", ENABLE_FAKE_SYSTEM_PROVIDERS: "true" })).toBe(true); expect(fakeSystemProvidersEnabled({ APP_ENV: "prod", ENABLE_FAKE_SYSTEM_PROVIDERS: "true" })).toBe(false); });
