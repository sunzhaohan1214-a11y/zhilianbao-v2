...
import { fakeSystemProvidersEnabled } from "@/modules/system/runtime";
...
describe("M3-007 system governance pure contracts", () => {
  ...
  it("requires an explicit non-production test-only fake-provider gate", () => {
    expect(fakeSystemProvidersEnabled({ APP_ENV: "test" })).toBe(false);
    expect(fakeSystemProvidersEnabled({ APP_ENV: "test", ENABLE_FAKE_SYSTEM_PROVIDERS: "true" })).toBe(true);
    expect(fakeSystemProvidersEnabled({ APP_ENV: "local", ENABLE_FAKE_SYSTEM_PROVIDERS: "true" })).toBe(true);
    expect(fakeSystemProvidersEnabled({ APP_ENV: "prod", ENABLE_FAKE_SYSTEM_PROVIDERS: "true" })).toBe(false);
    expect(fakeSystemProvidersEnabled({ APP_ENV: "test", NODE_ENV: "production", ENABLE_FAKE_SYSTEM_PROVIDERS: "true" })).toBe(false);
    expect(fakeSystemProvidersEnabled({ APP_ENV: "prod", NODE_ENV: "test", ENABLE_FAKE_SYSTEM_PROVIDERS: "true" })).toBe(false);
  });
  it("reports backup policy without inventing policy evidence", () => { const now = new Date("2026-08-28T12:00:00.000Z"); const row = { status: "SUCCEEDED", backupType: "MANUAL", completedAt: new Date("2026-08-28T00:00:00.000Z"), snapshotAt: new Date("2026-08-28T00:00:00.000Z"), retentionUntil: null, requestedAt: new Date("2026-08-28T00:00:00.000Z") } as never; expect(backupPolicyCompliance(false, [row], now).compliance).toBe("UNKNOWN"); expect(backupPolicyCompliance(true, [row], now)).toMatchObject({ ageHours: 12, compliance: "UNKNOWN", checks: { rpo24h: { status: "PASS" }, weeklyFull: { status: "UNKNOWN" } } }); expect(backupPolicyCompliance(true, [], now).compliance).toBe("DEGRADED"); });
