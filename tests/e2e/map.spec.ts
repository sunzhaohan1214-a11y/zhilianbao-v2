...
test("ordinary internal uses list-first enterprise/member maps with safe no-key degradation and no GPS", async ({ page }) => {
  ...
  await areaLink.click(); await expect(page.getByText("定位待完善", { exact: true })).toBeVisible(); await expect(page.getByRole("link", { name: "查看全县" })).toBeVisible();
  await page.goto("/resources/enterprises"); await expect(page.getByRole("heading", { name: "企业名录" })).toBeVisible();
  await page.goto("/resources/members"); await expect(page.getByRole("heading", { name: "团员" })).toBeVisible(); await page.getByRole("link", { name: "地图", exact: true }).click(); await expect(page.getByRole("heading", { name: "团员地图" })).toBeVisible(); await expect(page.getByText("展示派出单位地域分布，不是团员实时位置。")).toBeVisible(); await expect(page.getByText("E2E 派出单位", { exact: true })).toBeVisible(); await expect(page.getByText("3 人", { exact: true })).toBeVisible();
