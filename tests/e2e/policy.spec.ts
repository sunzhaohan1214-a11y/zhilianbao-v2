...
test("admin creates, extracts, confirms and publishes; member reads file; replacement never auto-restores", async ({ page }) => {
  ...
  await expect(page.getByText("AI 智能解读，仅供内部辅助")).toBeVisible(); const access = page.waitForResponse((response) => response.url().includes("/api/v2/attachments/") && response.url().endsWith("/access")); await page.getByRole("button", { name: "新政策.pdf" }).click(); expect((await access).status()).toBe(200);
