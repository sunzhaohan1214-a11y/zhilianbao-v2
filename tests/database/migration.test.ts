...
const service = new MigrationService(repository, storage, new FakeCleanScanner());
...
async function updateManifestEntry(root: string, relativePath: string, content: string, count?: number): Promise<void> {
  ...
  if (count !== undefined) manifest.files[relativePath].count = count;
...
describe("M3-006 Actual Apply on real MySQL", () => {
  it("keeps dry-run read-only and then writes real V2 targets, maps, attachments, and no historical Outbox", async () => {
    ...
    expect(attachment).toMatchObject({ isTemporary: false, uploadStatus: "UPLOADED", scanStatus: "PASSED", detectedMimeType: "application/pdf", detectedFileType: "pdf", sha256: "80b6aae12698eee2d1273b90fb6b49932f9c15b3102850f735c3af70f4eb26aa" });
    expect(Number(attachment.actualSizeBytes)).toBe(370);
  ...
  it("reruns the same provider without duplicate targets and preserves every target id", async () => {
    ...
    const beforeCounts = { people: await prisma.person.count(), enterprises: await prisma.enterprise.count(), demands: await prisma.demand.count(), progresses: await prisma.demandProgress.count(), reimbursements: await prisma.reimbursement.count(), attachments: await prisma.attachment.count(), attachmentLinks: await prisma.attachmentLink.count() };
    ...
    expect({ people: await prisma.person.count(), enterprises: await prisma.enterprise.count(), demands: await prisma.demand.count(), progresses: await prisma.demandProgress.count(), reimbursements: await prisma.reimbursement.count(), attachments: await prisma.attachment.count(), attachmentLinks: await prisma.attachmentLink.count() }).toEqual(beforeCounts);
  ...
  it("does not advance the Enterprise map when a changed mutable source needs review", async () => {
    const temporary = await mkdtemp(path.join(tmpdir(), "zlb-migration-enterprise-drift-"));
    try {
      await cp(fixture, temporary, { recursive: true });
      const enterprisePath = path.join(temporary, "entities/enterprises.ndjson");
      const changed = (await readFile(enterprisePath, "utf8")).replace("样本路1号", "样本路1号变更");
      await writeFile(enterprisePath, changed);
      await updateManifestEntry(temporary, "entities/enterprises.ndjson", changed);
      const beforeMap = await prisma.legacyMigrationMap.findUniqueOrThrow({ where: { sourceSystem_sourceEntity_sourceId: { sourceSystem: "ZHILIANBAO_V1", sourceEntity: "ENTERPRISE", sourceId: "ENTERPRISE-001" } } });
      const beforeTarget = await prisma.enterprise.findUniqueOrThrow({ where: { id: beforeMap.targetId } });
      const changedResult = await apply(temporary);
      expect(changedResult.records).toEqual(expect.arrayContaining([expect.objectContaining({ sourceEntity: "ENTERPRISE", sourceId: "ENTERPRISE-001", action: "REVIEW", targetId: beforeMap.targetId })]));
      expect(changedResult.issues).toEqual(expect.arrayContaining([expect.objectContaining({ code: "MIGRATION_SOURCE_CHANGED_REQUIRES_REVIEW", sourceId: "ENTERPRISE-001" })]));
      expect(await prisma.enterprise.findUniqueOrThrow({ where: { id: beforeMap.targetId } })).toMatchObject({ address: beforeTarget.address });
      expect(await prisma.legacyMigrationMap.findUniqueOrThrow({ where: { sourceSystem_sourceEntity_sourceId: { sourceSystem: "ZHILIANBAO_V1", sourceEntity: "ENTERPRISE", sourceId: "ENTERPRISE-001" } } })).toMatchObject({ targetId: beforeMap.targetId, sourceFingerprint: beforeMap.sourceFingerprint });
    } finally {
      await rm(temporary, { recursive: true, force: true });
    }
  });

  it("does not advance the Demand map when a main field changes without a formal update", async () => {
    const temporary = await mkdtemp(path.join(tmpdir(), "zlb-migration-demand-drift-"));
    try {
      await cp(fixture, temporary, { recursive: true });
      const demandPath = path.join(temporary, "entities/demands.ndjson");
      const changed = (await readFile(demandPath, "utf8")).replace("待对接样本需求", "待对接样本需求变更");
      await writeFile(demandPath, changed);
      await updateManifestEntry(temporary, "entities/demands.ndjson", changed);
      const beforeMap = await prisma.legacyMigrationMap.findUniqueOrThrow({ where: { sourceSystem_sourceEntity_sourceId: { sourceSystem: "ZHILIANBAO_V1", sourceEntity: "DEMAND", sourceId: "DEMAND-001" } } });
      const beforeTarget = await prisma.demand.findUniqueOrThrow({ where: { id: beforeMap.targetId } });
      const changedResult = await apply(temporary);
      expect(changedResult.records).toEqual(expect.arrayContaining([expect.objectContaining({ sourceEntity: "DEMAND", sourceId: "DEMAND-001", action: "REVIEW", targetId: beforeMap.targetId })]));
      expect(changedResult.issues).toEqual(expect.arrayContaining([expect.objectContaining({ code: "MIGRATION_SOURCE_CHANGED_REQUIRES_REVIEW", sourceId: "DEMAND-001" })]));
      expect(await prisma.demand.findUniqueOrThrow({ where: { id: beforeMap.targetId } })).toMatchObject({ title: beforeTarget.title });
      expect(await prisma.legacyMigrationMap.findUniqueOrThrow({ where: { sourceSystem_sourceEntity_sourceId: { sourceSystem: "ZHILIANBAO_V1", sourceEntity: "DEMAND", sourceId: "DEMAND-001" } } })).toMatchObject({ targetId: beforeMap.targetId, sourceFingerprint: beforeMap.sourceFingerprint });
    } finally {
      await rm(temporary, { recursive: true, force: true });
    }
  });
  ...
  it("rejects executable signatures and declared MIME mismatches without creating Attachment or Link", async () => {
    const temporary = await mkdtemp(path.join(tmpdir(), "zlb-migration-unsafe-attachments-"));
    try {
      await cp(fixture, temporary, { recursive: true });
      const blobs = path.join(temporary, "attachments/blobs");
      const executable = Buffer.from("MZ executable payload disguised as a PDF");
      const pdf = await readFile(path.join(blobs, "sample-policy.pdf"));
      await writeFile(path.join(blobs, "unsafe.pdf"), executable);
      await writeFile(path.join(blobs, "mime-mismatch.pdf"), pdf);
      const attachmentManifestPath = path.join(temporary, "attachments/manifest.ndjson");
      const rows = (await readFile(attachmentManifestPath, "utf8")).trimEnd().split("\n");
      rows.push(JSON.stringify({ sourceAttachmentId: "ATTACHMENT-004", sourceEntity: "ANNOUNCEMENT", sourceId: "ANNOUNCEMENT-001", relativePath: "unsafe.pdf", sha256: createHash("sha256").update(executable).digest("hex"), size: executable.byteLength, originalFilename: "伪装文件.pdf", declaredMimeType: "application/pdf" }));
      rows.push(JSON.stringify({ sourceAttachmentId: "ATTACHMENT-005", sourceEntity: "ANNOUNCEMENT", sourceId: "ANNOUNCEMENT-001", relativePath: "mime-mismatch.pdf", sha256: createHash("sha256").update(pdf).digest("hex"), size: pdf.byteLength, originalFilename: "类型不匹配.pdf", declaredMimeType: "image/png" }));
      const changedManifest = `${rows.join("\n")}\n`;
      await writeFile(attachmentManifestPath, changedManifest);
      await updateManifestEntry(temporary, "attachments/manifest.ndjson", changedManifest, rows.length);
      const before = { attachments: await prisma.attachment.count(), links: await prisma.attachmentLink.count() };
      const result = await apply(temporary);
      expect(result.issues.filter(({ code, sourceId }) => code === "MIGRATION_ATTACHMENT_SCAN_REJECTED" && ["ATTACHMENT-004", "ATTACHMENT-005"].includes(sourceId))).toHaveLength(2);
      expect(await prisma.legacyMigrationMap.count({ where: { sourceSystem: "ZHILIANBAO_V1", sourceEntity: "ATTACHMENT", sourceId: { in: ["ATTACHMENT-004", "ATTACHMENT-005"] } } })).toBe(0);
      expect({ attachments: await prisma.attachment.count(), links: await prisma.attachmentLink.count() }).toEqual(before);
    } finally {
      await rm(temporary, { recursive: true, force: true });
    }
  });
