...
import { CURRENT_SCHEMA_VERSION, currentAppVersion, currentRuntimeEnvironment } from "./runtime"; import type { SystemMutationContext } from "./types";
...
export class BackupService {
  ...
  private async request(input: Input & { type: BackupType; reason: string; idempotencyKey: string }) {
    ...
    try { id = await this.prisma.$transaction(async (tx) => { const replay = await findSystemCommand(tx, { actorPersonId: input.actor.personId, action, keyHash, payloadHash }); if (replay) return (replay.responseJson as { backupRecordId: string }).backupRecordId;
      const row = await tx.backupRecord.create({ data: { provider: providerHealth.provider, backupType: input.type, sourceEnvironment: currentRuntimeEnvironment(), status: "REQUESTED", reason: input.reason, schemaVersion: CURRENT_SCHEMA_VERSION, appVersion: currentAppVersion(), createdByPersonId: input.actor.personId } }); const response = { backupRecordId: row.id } as Prisma.InputJsonValue;
  ...
  async ensureSnapshotStarted(id: string) {
    ...
    try { const result = await this.provider.createSnapshot({ backupType: row.backupType, reason: row.reason, idempotencyKey: row.id }); return this.prisma.backupRecord.update({ where: { id }, data: { providerBackupId: result.providerBackupId, status: result.status, snapshotAt: result.snapshotAt, retentionUntil: result.retentionUntil, completedAt: result.status === "RUNNING" ? null : result.snapshotAt, errorCode: result.errorCode } }); }
  ...
  private providerCatalogMetadata(item: ProviderBackup) { return { providerBackupId: item.providerBackupId, backupType: item.backupType, sourceEnvironment: item.sourceEnvironment, ...this.providerStatusMetadata(item) }; }
}
