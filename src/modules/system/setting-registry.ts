...
export const SYSTEM_SETTING_REGISTRY = {
  ...
  "demand.review_sla_normal_workdays": { type: "INTEGER", default: 3, editable: true, riskLevel: "HIGH", schema: z.number().int().min(1).max(30), description: "普通需求审核时限（工作日）。" },
  "demand.review_sla_urgent_workdays": { type: "INTEGER", default: 1, editable: true, riskLevel: "HIGH", schema: z.number().int().min(1).max(30), description: "紧急需求审核时限（工作日）。" },
