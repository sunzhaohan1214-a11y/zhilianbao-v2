...
export class FakeMaintenanceProvider implements MaintenanceProvider {
  ...
  async exit(operationId: string) { if (!this.current && this.completed === operationId) return; if (this.current !== operationId) throw new Error("MAINTENANCE_OPERATION_MISMATCH"); this.current = undefined; this.completed = operationId; }
}
