...
export const boundaryActivateSchema = z.object({ reason: z.string().trim().min(1).max(500), confirmation: z.literal("ACTIVATE"), confirm: z.literal(true), expectedCurrentBoundaryId: z.uuid().nullable(), expectedTargetVersion: z.number().int().positive(), previewToken: z.string().length(64) }).strict();
