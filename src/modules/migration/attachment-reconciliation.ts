...
export type AttachmentPreviewResult = { record: LegacyAttachmentManifestRecord; status: "COPIED" | "MISSING" | "CORRUPTED" | "HASH_MISMATCH"; actualSha256?: string; actualSize?: number; issue?: MigrationPreviewIssue };
...
export async function reconcileSourceAttachment(provider: LegacySourceProvider, record: LegacyAttachmentManifestRecord): Promise<AttachmentPreviewResult> {
  try {
    ...
    return { record, status: "COPIED", actualSha256, actualSize: buffer.length };
  } catch (error) {
