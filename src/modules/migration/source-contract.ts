...
const schemas = {
  ...
  PERSON: z.object({ ...base, name: z.string().trim().min(1).max(80), phone: z.string().trim().max(30).optional(), memberKind: z.enum(["CURRENT", "ALUMNI_PLATFORM", "ALUMNI_HISTORICAL"]), currentEmploymentConfirmed: z.boolean().default(false), accountEligible: z.boolean().default(false), batchName: z.string().trim().max(100).optional(), startDate: z.iso.date().optional(), endDate: z.iso.date().optional() }).strict(),
  ENTERPRISE: z.object({ ...base, name: z.string().trim().min(1).max(200), responsibleAreaName: z.string().trim().min(1).max(200), address: z.string().trim().min(1).max(500), creditCode: z.string().trim().max(32).optional(), legalRepresentative: z.string().trim().max(80).optional(), introduction: z.string().trim().max(5000).optional(), mainProducts: z.string().trim().min(1).max(5000), latitude: z.number().min(-90).max(90).optional(), longitude: z.number().min(-180).max(180).optional(), contactName: z.string().trim().max(80).optional(), contactPhone: z.string().trim().max(30).optional(), primaryContactConfirmed: z.boolean().default(false) }).strict(),
