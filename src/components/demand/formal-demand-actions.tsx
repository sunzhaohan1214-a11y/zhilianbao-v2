  ...
  return (
    <div className="space-y-4">
      {canEdit && <form onSubmit={updateDraft} className={card}>
        <div><h2 className="text-lg font-semibold">编辑草稿</h2><p className="mt-1 text-sm text-slate-500">草稿或退回修改状态可编辑；联系人变化会同步刷新未发布快照。</p></div>
        <label className="block text-sm font-medium">搜索企业<input aria-label="编辑企业名称搜索" placeholder="输入名称可更换企业" className={field} onChange={(event) => void searchEnterprise(event.target.value).catch((error) => setMessage(error instanceof Error ? error.message : "企业搜索失败"))} /></label>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="text-sm font-medium">本次联系人<select name="selectedContactId" required defaultValue={enterprise.id === demand.enterprise.id ? demand.selectedContact.id : ""} key={enterprise.id} className={field}><option value="">请选择有效联系人</option>{activeContacts.map((contact) => <option key={contact.id} value={contact.id}>{contact.name} · {contact.phone}</option>)}</select></label>
