  ...
  return (
    <section className="rounded-2xl border border-indigo-200 bg-indigo-50 p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <p className="rounded-full bg-white px-3 py-1 text-xs text-indigo-700">在任认领截止：{data.claimDeadlineAt ? new Date(data.claimDeadlineAt).toLocaleString("zh-CN") : "未设置"}</p>
      </div>
      ...
      {data.canManage && !data.demandAlreadyClaimed && <div className="mt-5 space-y-4 border-t border-indigo-200 pt-4">
        ...
        {data.runHistory.length > 0 && <details className="rounded-xl bg-white p-4"><summary className="cursor-pointer font-medium">推荐运行历史</summary><ul className="mt-3 space-y-2 text-xs text-slate-600">{data.runHistory.map((runItem) => <li key={runItem.id}>{runItem.stage} · {runItem.status} · 候选 {runItem.candidateCount} 人 · {new Date(runItem.createdAt).toLocaleString("zh-CN")}{runItem.errorCategory ? ` · ${runItem.errorCategory}` : ""}</li>)}</ul></details>}
      </div>}
