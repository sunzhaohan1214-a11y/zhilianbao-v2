...
const scanStatusLabel: Record<string, string> = { PENDING: "待安全检查", SCANNING: "安全检查中", PASSED: "安全检查通过", REJECTED: "文件已拒绝", FAILED: "安全检查失败" };
const ownerExitStatusLabel: Record<string, string> = { PENDING_REVIEW: "待审核", APPROVED: "已同意", RETURNED: "已退回", CANCELED: "已取消" };
  ...
  return (
    <div className="space-y-5">
      <article className={card}>
        ...
        {overview.progresses.length === 0 ? <p className="text-sm text-slate-500">暂无进展记录。</p> : <ol className="space-y-4">{overview.progresses.map((item) => <li key={item.id} className="rounded-xl border border-slate-200 p-4"><div className="flex flex-wrap justify-between gap-2 text-xs text-slate-500"><span>{sourceLabel[item.sourceType] ?? "其他记录人"} · {item.createdByPerson.name}{item.representedPerson ? `（代 ${item.representedPerson.name}）` : ""}</span><time>{shanghai(item.createdAt)}</time></div><p className="mt-3 whitespace-pre-wrap text-sm leading-6"><span className="font-medium">当前进展：</span>{item.currentProgress}</p><p className="mt-2 whitespace-pre-wrap text-sm leading-6"><span className="font-medium">下一步：</span>{item.nextStep}</p>{item.attachments.length > 0 && <ul className="mt-3 text-sm text-blue-700">{item.attachments.map((attachment) => <li key={attachment.id}>附件：{attachment.originalFilename} · {scanStatusLabel[attachment.scanStatus] ?? "检查状态待确认"}</li>)}</ul>}</li>)}</ol>}
      </article>
