...
export function MonthlyReportConsole({ areas, batches, countyWide, canDownload }: { areas: Option[]; batches: Option[]; countyWide: boolean; canDownload: boolean }) {
  ...
  return <div className="space-y-6">
    ...
    {preview && <>
      ...
      <section className="rounded-3xl border border-warning/20 bg-warning-soft p-5"><h2 className="font-semibold text-foreground">数据质量说明</h2>{preview.warnings.length === 0 ? <p className="mt-2 text-sm text-warning">本次预览未发现需要说明的历史归属问题。</p> : <ul className="mt-3 space-y-2 text-sm text-foreground">{preview.warnings.map((warning) => <li key={warning.code}>{warning.message}（{warning.count} 条）</li>)}</ul>}</section>
    </>}
  </div>;
}
