...
export function MapCanvas({ mapKey, boundaries, points, emptyMessage = "暂无可显示的地图数据" }: { mapKey: string | null; boundaries: BoundaryShape[]; points: MapPoint[]; emptyMessage?: string }) {
  const ref = useRef<HTMLDivElement>(null); const [status, setStatus] = useState<"idle" | "loading" | "ready" | "error">("idle");
  useEffect(() => {
    if (!mapKey || !ref.current) return; const renderer = new TencentMapRenderer(mapKey); let active = true; setStatus("loading");
    renderer.render({ container: ref.current, boundaries, points }).then(() => active && setStatus("ready")).catch(() => active && setStatus("error"));
    return () => { active = false; renderer.destroy(); };
  }, [mapKey, boundaries, points]);
  ...
  if (!boundaries.length && !points.length) return <div className="grid min-h-72 place-items-center rounded-3xl border border-dashed border-slate-300 bg-slate-50 p-6 text-sm text-slate-500">{emptyMessage}</div>;
