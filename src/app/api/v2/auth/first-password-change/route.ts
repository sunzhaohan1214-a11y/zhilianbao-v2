...
import { completeFirstActivation } from "@/modules/identity/auth-service";
import { firstPasswordChangeSchema } from "@/modules/identity/schemas";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const body = firstPasswordChangeSchema.parse(await request.json());
    const result = await completeFirstActivation({ current, ...body, context });
