...
export async function POST(request:NextRequest,{params}:{params:Promise<{id:string}>}){const context=buildAuthRequestContext(request);try{const{actor,lifecycle}=await formalDemandRequestContext(request,true);return apiSuccess(await lifecycle.transferOwner({actor,context,demandId:(await params).id,body:await request.json(),idempotencyKey:request.headers.get("Idempotency-Key")}),context.requestId);}catch(error){return apiError(error,context.requestId);}}
