...
import { reviewEnterpriseChangeRequestSchema } from "@/modules/enterprise/schemas";
...
export async function POST(request: NextRequest, route: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const body = reviewEnterpriseChangeRequestSchema.parse(await request.json());
    const { id } = await route.params;
    return apiSuccess(await service.reviewChangeRequest({ actor, context, requestId: id, ...body }), context.requestId);
