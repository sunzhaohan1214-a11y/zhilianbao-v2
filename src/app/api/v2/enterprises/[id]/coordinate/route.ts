...
import { coordinateSchema } from "@/modules/enterprise/schemas";
...
export async function POST(request: NextRequest, route: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const body = coordinateSchema.parse(await request.json());
    const { id } = await route.params;
    return apiSuccess(await service.coordinate({ actor, context, enterpriseId: id, ...body }), context.requestId);
