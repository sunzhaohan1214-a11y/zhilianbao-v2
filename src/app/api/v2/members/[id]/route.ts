...
export async function GET(request: NextRequest, route: { params: Promise<{ id: string }> }) {
  ...
  try { const { actor, members } = await foundationRequestContext(request); return apiSuccess(await members.detail({ actor, context, personId: (await route.params).id }), context.requestId); }
