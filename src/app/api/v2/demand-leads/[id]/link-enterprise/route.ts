...
import { linkDemandLeadEnterpriseSchema } from "@/modules/demand/schemas";
...
export async function POST(request: NextRequest, route: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const body = linkDemandLeadEnterpriseSchema.parse(await request.json());
    const { id } = await route.params;
    return apiSuccess(await service.linkEnterprise({ actor, context, leadId: id, ...body }), context.requestId);
