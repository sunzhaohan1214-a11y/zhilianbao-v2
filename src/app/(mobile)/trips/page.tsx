...
export default async function TripsPage() {
  ...
  const groups = [
    { title: "今天", items: result.items.filter((trip) => trip.nodes.some((node) => new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Shanghai" }).format(node.plannedStartAt) === todayKey)) },
    { title: "即将开始", items: result.items.filter((trip) => trip.status === "PLANNED") },
    { title: "待补结果", items: result.items.filter((trip) => trip.status === "PENDING_RESULT") },
    { title: "历史", items: result.items.filter((trip) => ["COMPLETED", "CANCELED"].includes(trip.status)) },
  ];
