...
export default async function MembersPage({ searchParams }: { searchParams: Promise<{ kind?: string; keyword?: string }> }) {
  ...
  return <section><h1 className="text-2xl font-semibold">团员</h1><p className="mt-2 text-sm text-slate-500">Person 永久保留，多批次履历不会复制人员。</p>
    ...
    <form className="mt-4 flex gap-2"><input type="hidden" name="kind" value={kind} /><input name="keyword" defaultValue={params.keyword} placeholder="姓名或电话" className="min-w-0 flex-1 rounded-xl border border-slate-200 bg-white p-3" /><button className="rounded-xl bg-slate-900 px-4 text-sm text-white">搜索</button></form>
