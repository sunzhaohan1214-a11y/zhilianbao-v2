...
export default async function MonthlyReportPage() {
  ...
  return <main className="min-h-dvh bg-background px-4 py-8 sm:px-8"><div className="mx-auto max-w-7xl"><div className="mb-7 flex items-end justify-between gap-4"><div><p className="text-sm font-medium text-brand">数据与报表</p><h1 className="mt-1 text-3xl font-semibold tracking-tight">月度工作台账</h1><p className="mt-2 text-sm text-muted">结构化正式数据 · 上海自然月 · 历史时点可审计</p></div><Link href={actor.capabilities.has("admin.shell.access") ? "/admin" : "/me"} className="min-h-11 content-center text-sm font-medium text-brand">返回</Link></div><MonthlyReportConsole areas={options.areas} batches={options.batches.map((item) => ({ id: item.id, name: item.name }))} countyWide={options.scope.countyWide} canDownload={canDownloadMonthlyReport(actor)} /></div></main>;
}
