...
自由文本同样必须递归清洗大陆手机号、15/18 位身份证号和 email，并替换为稳定占位符。清洗只作用于送 Provider 的 DTO，不改写 MySQL 正式事实或保存的 evidence snapshot。
...
输入只包含需求事实、能力画像、当前主责数和近期活动摘要；不包含手机号、身份证号、email、联系人、求助或报销数据。即使这些 PII 混入标题、描述、标签或 evidence 自由文本，也必须先由 Sanitizer 递归替换。
