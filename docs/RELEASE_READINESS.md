...
Migration evidence has an additional fail-closed FULL contract. It requires `snapshotKind=FULL`, `FULL_REHEARSAL/COMPLETED`, a SHA-256 manifest digest, and count/SHA-256 rows for every current V1 contract entity file plus `attachments/manifest.ndjson`. The attachment inventory must bind to that exact manifest row and prove every source attachment was copied and re-read with matching hash, with zero issues. Evidence must identify distinct dry-run, apply batch/run and rerun batch/run executions, the dedicated target migration database, and zero unresolved BLOCKER or REVIEW issues. It must include exactly one row for every migration module and `ATTACHMENT`; each row must satisfy both source and attachment reconciliation equations, with zero failed, review or attachment-issue counts. All dry-run/apply/rerun/reconciliation booleans must still be true. SAMPLE labels, omitted files/modules, duplicate identities, mismatched digests/counts, or arithmetically invalid rows fail with `MIGRATION_EVIDENCE_INCOMPLETE`.
